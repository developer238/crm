...
export class TrackingService {
	...
	async setFlag(
		...
		await this.db.appSetting.upsert({
			where: { projectId: currentProjectId() },
			create: { projectId: currentProjectId(), [column]: enabled },
	...
	async setCookieDays(userId: string, days: number): Promise<void> {
		...
		await this.db.appSetting.upsert({
			where: { projectId: currentProjectId() },
			create: { projectId: currentProjectId(), trackingCookieDays: days },
	...
	async addDomain(
		...
		try {
			const domain = await this.db.trackedDomain.create({
				data: { projectId: currentProjectId(), host, scope: input.scope },
			});
