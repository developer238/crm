...
export class TrackingFilingService {
	...
	async file(submission: {
		...
		const suppressed = await this.suppressed(
			email,
			domain,
		...
		const existing = await this.db.contact.findUnique({
			where: {
				projectId_email: { projectId: submission.projectId, email },
			},
			select: { id: true },
		});
		...
		try {
			...
			const raced = await this.raced(error, email, submission.projectId);
