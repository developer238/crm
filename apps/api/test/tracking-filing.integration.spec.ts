...
async function clean() {
	...
	await db.contact.deleteMany({
		where: { projectId, email: { endsWith: `@${domain}` } },
	});
	await db.contact.deleteMany({
		where: { projectId, email: `free-${suffix}@gmail.com` },
	});
...
beforeAll(async () => {
	fixture = await createTestProject("tracking-filing");
...
describe("filing a form submission", () => {
	...
	it("refuses an address a rep has deleted", async () => {
		...
		await db.suppressedContact.create({ data: { projectId, email } });
	...
	it("refuses a suppressed domain", async () => {
		await db.suppressedDomain.create({ data: { projectId, domain } });
	...
	it("files a free-mail address as a contact with no company", async () => {
		...
		const contact = await db.contact.findUnique({
			where: {
				projectId_email: { projectId, email: `free-${suffix}@gmail.com` },
			},
	...
	it("gives the cap slot back to the delivery that lost the create", async () => {
		...
		const counter = await db.trackingCounter.findUnique({
			where: { projectId_key: { projectId, key: contactWindowKey() } },
			select: { value: true },
		});
	...
	it("attaches a second submission to the contact already there", async () => {
		...
		const count = await db.contact.count({ where: { projectId, email } });
		expect(count).toBe(1);
	});
